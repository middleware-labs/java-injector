...
// unixSocketClient is the shared implementation for Docker-compatible container
// runtime APIs (Docker, Podman) that serve JSON over a Unix socket.
type unixSocketClient struct {
	name       string
	httpClient *http.Client
}

func newUnixSocketClient(name, socketPath string) unixSocketClient {
...
func (u *unixSocketClient) ping(ctx context.Context) bool {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, "http://"+u.name+"/_ping", nil)
	if err != nil {
		return false
	}
...
func (u *unixSocketClient) InspectBatch(ctx context.Context, ids []string) (map[string]ContainerMeta, error) {
	result := make(map[string]ContainerMeta, len(ids))
	for _, id := range ids {
		meta, err := u.inspect(ctx, id)
		if err != nil {
			continue
		}
		result[id] = meta
...
func (u *unixSocketClient) inspect(ctx context.Context, id string) (ContainerMeta, error) {
	url := fmt.Sprintf("http://%s/containers/%s/json", u.name, id)
	...
	if resp.StatusCode != http.StatusOK {
		return ContainerMeta{}, fmt.Errorf("%s inspect %s: status %d", u.name, id, resp.StatusCode)
	}
